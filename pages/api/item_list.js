...
export default async(req, res) => {
	...
	try {
        ...
        const sql = `select A.item_id, A.subject, A.summary, A.thumbnail, A.wdate, A.love as loveCount, A.pathname, ifnull(B.commentCount, 0) as commentCount
                    ...
                    on A.item_id=B.item_id where A.kind=:kind and open=1 ${search ? 'and ( subject like :search or content like :search ) ' : ''} order by wdate desc limit ${ Number(startpoint) * 12}, 12
        `
        const resItem = await sequelizeSlave.query(sql, {replacements, type: sequelizeSlave.QueryTypes.SELECT })

        const sqlCount = `select count(*) as count from item where kind=:kind and open=1 ${search ? 'and ( subject like :search or content like :search ) ' : ''}`
